...
class WSConnectionManager:
    ...
    async def disconnect(self, connection_id: str) -> None:
        ...
        try:
            ...
            _logger.debug(f'安全忽略关闭WebSocket异常：{connection_id}，信息：{str(e)}')
    ...
    async def _local_broadcast_text(self, message: str, *, group: str | None = None,
        ...
        # 优化：忽略断开导致的异常，防止并发 gather 失败引发雪崩
        await gather(*(self.send_text(cid, message) for cid in targets), return_exceptions=True)

    async def _local_broadcast_json(self, data: Any, *, group: str | None = None,
                                    exclude: set[str] | None = None) -> None:
        """仅执行本地JSON广播"""
        targets = self._resolve_targets(group, exclude)
        if not targets:
        ...
        # 优化：忽略断开导致的异常，防止并发 gather 失败引发雪崩
        await gather(*(self.send_json(cid, data) for cid in targets), return_exceptions=True)
