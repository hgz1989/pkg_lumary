...
通过底层的 `WrapAPIRoute` 拦截器，开发者只需在路由函数中直接返回业务模型，框架会在序列化前自动将其包装为企业级的 `APIResponse`，并且 **完美支持 Swagger OpenAPI Schema 推导**！
- 支持普通数据直接返回。
- 支持 `return data, extra` 元组自动解包并生成带有扩展字段的结构。
- 支持 `return data, extra, message` 三元组自动解包，实现自定义成功提示信息。
